...
func (s *Store) CreateRound(ctx context.Context, input RoundInput) (Round, error) {
	...
	res, err := s.db.ExecContext(ctx, `
		INSERT INTO rounds(slug, name, mode, status, require_locked_agents, initial_balance_cents, starts_at, ends_at, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, input.Slug, input.Name, input.Mode, input.Status, boolInt(input.RequireLockedAgents), input.InitialBalanceCents, nullIfEmpty(input.StartsAt), nullIfEmpty(input.EndsAt), now, now)
	if err != nil {
		return Round{}, err
	}
...
func (s *Store) ListRounds(ctx context.Context) ([]Round, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, slug, name, mode, status, require_locked_agents, initial_balance_cents, starts_at, ends_at, created_at, updated_at
		FROM rounds
		ORDER BY id
	`)
	if err != nil {
...
func (s *Store) SetRoundRequireLockedAgents(ctx context.Context, id int64, required bool) (Round, error) {
	_, err := s.db.ExecContext(ctx, "UPDATE rounds SET require_locked_agents = ?, updated_at = ? WHERE id = ?", boolInt(required), Now(), id)
	if err != nil {
		return Round{}, err
	}
	return s.GetRound(ctx, id)
...
func scanRound(row roundScanner) (Round, error) {
	...
	var requireLockedAgents int64
	err := row.Scan(&round.ID, &round.Slug, &round.Name, &round.Mode, &round.Status, &requireLockedAgents, &round.InitialBalanceCents, &startsAt, &endsAt, &round.CreatedAt, &round.UpdatedAt)
	round.RequireLockedAgents = scanBool(requireLockedAgents)
