...
func run(args []string, out io.Writer) error {
	...
	case "create-agent":
		...
		kind := fs.String("kind", "agent", "agent kind")
...
func (c client) seedDemo(out io.Writer) error {
	...
	for _, t := range teams {
		...
		var result agentTokenResponse
		if err := c.do("POST", fmt.Sprintf("/api/v1/admin/teams/%d/agents", t.ID), map[string]string{"slug": "default", "name": "Default Agent", "kind": "agent"}, &result); err != nil {
			return err
		}
		if err := c.do("POST", fmt.Sprintf("/api/v1/admin/rounds/%d/teams/%d/enroll", r.ID, t.ID), nil, nil); err != nil {
			return err
	...
	if len(createdAgents) > 0 {
		fmt.Fprintln(out, "Demo agent tokens for registered agents, shown once:")
	}
